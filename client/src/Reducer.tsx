...
export function appReducer(state: AppState, action: DispatchAction): AppState {
  ...
  const newState = (() => {
    switch (type) {
      ...
      case 'MIDI_PLAY_NOTE': {
        //console.log(args.get('midiNote'));
        console.log("MIDI_PLAY_NOTE", `ARGS: ${args}`);
        return state.set('midiTrigger', args.get('midiNote'));
        //return state;
      }
      case 'MIDI_STOP_NOTE': {
        console.log("MIDI_STOP_NOTE");
        return state.remove('midiTrigger');
        //return state;
      }
      default:
